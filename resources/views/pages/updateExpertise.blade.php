...
<script>
    ...
    <section class="section-service1">
        <form method="POST" action="{{ route('expertise.add') }}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="id" value="{{ $expertise->id }}" style="display: none">

                <div class="container-fluid">
             
                    
                    <div class="form-group mb-4">
                        <input type="text" name="id" class="form-control" value="{{$expertise->id}}" style="display: none">
                    </div>
                    <div class="col-lg-12">
                        <div class="form-group mb-4">
                            <label>Redigez la Description</label>
                            <textarea id="description"  name="description" class="form-control"  name="description" >{{ $expertise->description}}</textarea>
                            @error('description')
