...
<section class="service-odoo1">
    ...
    <form method="POST" action="/bigdata/edit" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="id" value="{{ $service_bigdata->id }}" style="display: none">

        <div class="mb-3">
            <label for="image" class="form-label">Image du service</label>
            <input type="file" name="image" class="form-control">
            
            @if($service_bigdata->path_image)
                <div class="mt-2">
                    <p>Image actuelle :</p>
                    <img src="{{ asset('storage/' . $service_bigdata->path_image) }}" alt="Image du service" style="max-width: 200px; max-height: 200px;">
                </div>
            @endif
        ...
        <div class="mb-3">
            ...
            <input type="text" name="titre" class="form-control" value="{{ $service_bigdata->titre }}"" required>
            @error('titre')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea id="description" name="description" class="form-control" rows="4" required>{{ $service_bigdata->description }}</textarea>
            @error('description')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>

        <button type="submit" class="btn btn-primary">Modifier BigData</button>
    </form>
</section>
@endsection
